...
const ProductPage = () => {
  ...
  // Intersection Observer setup for fade-in on scroll
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    setProducts(allProducts); // Only set once
  }, []);
  ...
  return (
    <div className="text-center bg-purple-50 font-sans text-gray-800 px-4 sm:px-8 md:px-16">
      {/* Header */}
      <header className="border-b py-4 mb-12">
      ...
      {/* Banner Section */}
      <section className="grid grid-cols-2 sm:grid-cols-4  gap-4 p-4 bg-gray-100 mt-6 my-16">
      ...
      <section className="mt-10 my-16">
        ...
        <p className="text-sm max-w-xl mx-auto mt-2 mb-6 px-2">
          Browse a wide selection of men’s and women’s fashion in one place.
        </p>

        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6 px-2 sm:px-8">
          {products.map((product, idx) => (
            <div
              key={product.id}
              ref={el => productRefs.current[idx] = el}
              className="relative bg-white p-3 rounded shadow hover:shadow-xl transform transition-transform duration-300 hover:scale-105 opacity-0 translate-y-10"
              style={{ transitionProperty: 'opacity, transform' }}
            >
              {/* Wishlist Heart */}
              <button
              ...
              <Link to={`/product/${product.id}`}>
                <img
                  src={product.image}
                  alt={product.name}
                  className="w-full h-64 sm:h-80 object-contain rounded cursor-pointer"
                />
                <h3 className="mt-3 font-semibold hover:underline hover:text-pink-500">
                  {product.name}
                </h3>
              </Link>
