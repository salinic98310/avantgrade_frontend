...
const WomenPage = () => {
  ...
  return (
    <div className="text-center font-sans text-gray-800 px-4 sm:px-6 md:px-10 lg:px-16 xl:px-24">
      {/* Header */}
      <header className="border-b py-4 sticky top-0 bg-white z-30">
        <div className="text-2xl font-bold">All Products</div>
        <nav className="mt-2 text-sm">
          <a href="/" className="mx-2 hover:underline hover:text-pink-500">
            Home /
          </a>
          <a href="/products" className="mx-1 hover:underline hover:text-pink-500">
            All Products
          </a>
        </nav>
      </header>
      ...
      {/* Banner Section */}
      <section className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-gray-100 mt-6">
        {categories.map((cat, i) => (
          <div
            key={i}
            ref={(el) => (bannerRefs.current[i] = el)}
            className="relative group overflow-hidden rounded-lg shadow-lg transform transition-transform duration-700 hover:scale-105 opacity-0 translate-y-10"
      ...
      <section className="mt-10">
        <h2 className="text-3xl font-semibold">Explore All Styles</h2>
        <p className="text-sm max-w-xl mx-auto mt-2 mb-6 px-2 sm:px-0">
          Browse a wide selection of men’s and women’s fashion in one place.
        </p>

        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 px-2 sm:px-4 md:px-8">
          {products.map((product, idx) => (
            <div
              key={product.id}
              ref={(el) => (productRefs.current[idx] = el)}
              className="relative bg-white p-3 rounded shadow hover:shadow-xl transform transition-transform duration-700 hover:scale-105 opacity-0 translate-y-10"
              style={{ transitionProperty: 'opacity, transform' }}
            >
              {/* Wishlist Heart */}
              <button
